  ...
  return (
    <div
      ...
      className={`
        ...
        bg-amber-100 text-teal-800 font-bold rounded-sm shadow-[1px_2px_2px_rgba(0,0,0,0.3)] border border-amber-200
      ...
      `}
    >
      <span className={`z-10 leading-none pb-[0.1em]${tile.isBlank ? ' text-coral-400' : ''}`}>{tile.displayChar}</span>
      <span className={`absolute font-bold text-teal-800 leading-none ${pointSizeClass}`}>
        {tile.value}
      </span>
      {/* Visual indicator for blank tile */}
      {tile.isBlank && (
        <div className="absolute inset-0 border-2 border-mist-400/70 rounded-sm pointer-events-none"></div>
      )}
    </div>
  );
};
